...
def subject_detail_anime_select(conn, where_sql):
    select_sql = """ select
		...
		sale_date, send_out_week, end_date, time_length, topic_length, 
		official_website, TV_station, copyright, extends, intro, 
...
def subject_detail_anime_insert(conn, dto: subject_detail_anime_dto.SubjectDetailAnimeDTO):
    insert_sql = """ insert into subject_detail_anime(
		...
		picture, chinese_name, alias, release_date, sale_date, 
    ...
    params = (
        ...
        dto.picture, dto.chinese_name, dto.alias, dto.release_date, dto.sale_date,
        dto.send_out_week, dto.end_date, dto.time_length, dto.topic_length, dto.official_website,
        dto.TV_station, dto.copyright, dto.extends, dto.intro, dto.topic_list_id,
...
def subject_detail_anime_update(conn, dto: subject_detail_anime_dto.SubjectDetailAnimeDTO):
    update_sql = """
		...
		chinese_name= %s, alias= %s, release_date= %s, sale_date= %s, send_out_week= %s, 
		end_date= %s, time_length= %s, topic_length= %s, official_website= %s, TV_station= %s, 
		copyright= %s, extends= %s, intro= %s, topic_list_id= %s, comment_box_id= %s, 
		system_score_id= %s, system_tag_id= %s, do= %s, collect= %s, wish= %s, 
		on_hold= %s, dropped= %s, remark= %s, status= %s, creator= %s, 
		create_time= %s, updater= %s, update_time= %s
		where id = %s and optimistic = %s """
    params = (
        dto.subject_id, dto.bangumi_subject_id, dto.name, dto.type, dto.picture,
        dto.chinese_name, dto.alias, dto.release_date, dto.sale_date, dto.send_out_week,
